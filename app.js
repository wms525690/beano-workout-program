/* ================================================
   CORBEANO GOMEZ — WORKOUT PROGRAM APP
...
(function () {
  ...
  if (finishSave) {
    finishSave.addEventListener('click', function () {
      ...
      state.workouts[day] = { duration: elapsed, difficulty: difficulty, completedAt: new Date().toISOString() };
      saveLocalState(allState);
      ...
      if (startBtn) { startBtn.textContent = 'Workout Logged!'; startBtn.style.opacity = '0.5'; }
      var timerEl = document.getElementById('timer-' + day);
      if (timerEl) timerEl.style.display = 'none';
      var finishBtnEl = document.querySelector('.finish-workout-btn[data-day="' + day + '"]');
